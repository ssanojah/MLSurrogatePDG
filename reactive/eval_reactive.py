"""
Closed-loop evaluation of the reactive Transformer.

Rolls the policy out with a fixed dt until touchdown, timeout (--t_max),
fuel exhaustion or divergence, and classifies each IC. Writes
eval_config.json, summary.json, per_ic_results.csv and histograms.

Usage:
    python eval_reactive.py --run_dir runs/reactive_w16 \\
        --data_dir data/<dataset> --out_dir results/reactive_eval
"""
...
FIG_EXT = "svg"     # one figure per file, vector, no in-figure titles
FIG_DPI = 300

def _save(fig, out_dir, name):
    p = Path(out_dir) / f"{name}.{FIG_EXT}"
...
class ReactiveEvalConfig:
    ...
    # Integration
    dt: float = 0.8
    t_max: float = 80.0        # maximum simulation time [s]
...
def plot_terminal_histograms(results, out_dir, tilt_source='raw',
    ...
    panels = [
        ('terminal_speed',    'v_norm', r'$\|\mathbf{v}\|$  [m/s]', (), 99.0, data),
        ('terminal_vz',       'v_z',    r'$|v_z|$  [m/s]',
            ((THR_VZ_SOFT, f'soft {THR_VZ_SOFT:g}', '--'),
             (THR_VZ_HARD, f'hard {THR_VZ_HARD:g}', ':')), 99.0, data),
        ('terminal_altitude', 'r_z',    r'$r_z$  [m]', (), 99.0,
            {c: {'r_z': data[c]['r_z'][data[c]['landed'] < 0.5]}
             for c in PLOT_CLASSES}),
        ('terminal_mass',     'm_f',    r'$m_f$  [t]', (), None, data),
        (f'terminal_tilt_{tilt_source}', 'tilt', 'tilt from vertical  [deg]',
            ((theta_max_deg, f'{theta_max_deg:g}$^\\circ$ cone', '--'),),
            99.5, data),
        ('terminal_rh',       'r_h',    r'$\|\mathbf{r}_h\|$  [m]',
            ((THR_RH_SOFT, f'soft {THR_RH_SOFT:g}', '--'),), 99.0, data),
    ]
    ...
    for name, key, xlabel, thr, clip, src in panels:
        fig, ax = plt.subplots(figsize=(6.0, 4.2))
        _stacked_hist(ax, src, key, xlabel, '', thresholds=thr,
                      n_bins=n_bins, log_y=log_y, clip_percentile=clip)
        if name == 'terminal_altitude' and n_landed_omitted:
            ax.text(0.99, 0.78,
...
def plot_single_ic(result, out_dir, dt, which=('thrust', 'altitude', 'vz')):
    """Expert-vs-policy figures for one IC, one file each, named as in
    standalone_eval_v2: <quantity>_example_<class>_<ic_index>.svg
    """
    if not _HAVE_MPL:
        return []

    cls = result['classification']
    tag = f"example_{cls}_{result['ic_index']:04d}"

    x_pol = result['x_traj']
    n_steps = result['n_steps']
    t_pol_state = np.arange(len(x_pol)) * dt
    t_pol_cmd = np.arange(n_steps) * dt

    expert = np.load(result['filepath'])
    x_exp = expert['x_traj']
    u_exp = expert['u_traj']
    t_f = float(expert['t_f'])
    t_exp_state = np.linspace(0.0, t_f, len(x_exp))
    t_exp_cmd = np.linspace(0.0, t_f * (len(u_exp) - 1) / len(u_exp),
                            len(u_exp))

    s_exp = dict(color='tab:blue', lw=2, label='Expert')
    s_pol = dict(color='tab:red', lw=2, ls='--', label='Policy')
    paths = []

    if 'thrust' in which:
        fig, ax = plt.subplots(figsize=(6.0, 4.2))
        ax.step(t_exp_cmd, np.linalg.norm(u_exp, axis=1) * 1e3,
                where='post', **s_exp)
        ax.step(t_pol_cmd, result['T_mag_MN'] * 1e3, where='post', **s_pol)
        ax.step(t_pol_cmd, result['T_mag_raw_MN'] * 1e3, where='post',
                color='tab:red', alpha=0.35, lw=1, label='Policy raw')
        ...
        ax.grid(alpha=0.3)
        paths.append(_save(fig, out_dir, f'thrust_{tag}'))
    return paths
    ...
    if config.make_plots:
        ...
        plot_paths = plot_terminal_histograms(
            ...
            log_y=config.plot_log_y,
                ...
                # One example per outcome class, first occurrence
        seen = set()
        for r in results:
            c = r['classification']
            if c in PLOT_CLASSES and c not in seen:
                seen.add(c)
                plot_paths += plot_single_ic(r, out_dir, config.dt)
...
def main():
    parser = argparse.ArgumentParser(
        description='Reactive Transformer closed-loop evaluation',
    ...
    parser.add_argument('--out_dir', type=str, default='results/reactive_eval',
    ...
    parser.add_argument('--t_max', type=float, default=80.0,
                        help='Max simulation time [s] (default: 80)')
    ...
    print("RETALT1 PDG — Reactive Transformer Evaluation")
